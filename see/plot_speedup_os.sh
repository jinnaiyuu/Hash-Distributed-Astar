...
astar=astar.dat
data1=hdastar_1_1008_os.dat
#data2=hdastar_2_1008_os.dat
data4=hdastar_4_1008_os.dat
data8=hdastar_8_1008_os.dat
...
awk 'FNR==NR{astar[$1]=$2; next} (FNR+100)==NR{a[$1]=$2; next} (FNR+200)==NR{c[$1]=$2; next} \
(FNR+300)==NR{\
if (length(astar[$1]) != 0) \
if (length(a[$1]) != 0) \
if (length(c[$1]) != 0) \
if (length($4) != 0) \
print astar[$1], a[$1], c[$1], $2}' ${astar} ${data1} ${data4} ${data8} > speedup.dat

awk '{ astar += $1; single+= $2;  quadra+=$3; oct+=$4; } END{ printf("1 %f\n4 %f\n8 %f", astar/single, astar/quadra, astar/oct)}' \
speedup.dat > spd

gnuplot <<EOF
   set title "Speedup of JOHDA* compared to A*"
   set xlabel "Number of threads"
   set ylabel "Speedup"   
   set terminal postscript color
   plot "spd" using 1:2 with lp
   set output "analysis/speedup_of_johda_over_astar.ps"
