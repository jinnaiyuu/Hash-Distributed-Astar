...
#ifndef _HASHTBLBIG_HPP_
#define _HASHTBLBIG_HPP_
...
template<class Node> struct HashEntryBig {
	uint128_t hash;
	Node *next;
};

// HashTable implements a simple single-sized hash table.
template<class Key, class Node> class HashTableBig {
...
//		pthread_mutex_init(&m, NULL);
	...
	//       possibly null input?
	Node *find(Key &key) {
		uint128_t h = key.hash();
		uint128_t ind = h % buckets.size();
...
//		pthread_mutex_lock(&m);
		uint128_t hash = n->key().hash();
		uint128_t ind = hash % buckets.size();
