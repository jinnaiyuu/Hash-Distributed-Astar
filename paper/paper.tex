...
% bind effect
HDA*のノードの展開順は大きな流れとしてはA*のそれと一致するようである。しかしながら「幅」が存在し、その範囲内で展開順が前後する。この幅はスレッド数が大きい程広い。この幅は実行時間を通して大きくなっていくが、展開ノード数が増加する割合に対して小さい。その為、問題サイズが大きくなっても、展開ノード数に対する相対的な幅の大きさは変わらない。図\ref{fig:order_8_threads_easy}はより大きなインスタンス
