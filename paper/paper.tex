...
12, 24 threadsのときに探索オーバーヘッドが若干生じる。Zobrist Hashは、スレッドの数が2^nでない場合、スレッドの数で除算しなければならない。このとき、ノードの分配は不均一になる。不均一性はZobrist Hashのビット列の長さとトレードオフになる。長いほどスレッド間の分配は均一になるが、各ノードに必要な情報が増える。本実験では4 Bytes (4294967296)をビット列の長さとした。理論的には、24スレッド実行において最も分配の多いスレッドと最も少ないスレッドのノードの比率は178956971:178956970である。ほぼ無視出来る差であるように考えられるが、探索オーバーヘッドが生じている。この探索オーバーヘッドの原因やビット列の長さとのトレードオフは今後の研究課題である。
